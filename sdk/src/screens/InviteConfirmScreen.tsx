...
type Nav = StackNavigationProp<AuthStackParamList, 'InviteConfirm'>;
...
export function InviteConfirmScreen() {
  ...
  const { inviteCode, mobileNumber, setMobileNumber, setOtpDigits, setOtpError, setOtpCountdown } = useMGLApp();

  const companyName = INVITE_CODES_DB[inviteCode] || 'Fleet Operator';

  return (
    <View style={{ paddingHorizontal: 24, paddingVertical: 32, gap: 16 }}>
      ...
      <TouchableOpacity
        onPress={() => { setOtpDigits(Array(6).fill('')); setOtpError(''); setOtpCountdown(30); navigation.navigate('OnboardingOtp'); }}
