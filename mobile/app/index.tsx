...
import {
  ...
  LogOut, Home, Wifi, Route, CheckCircle,
...
export default function Page() {
  ...
  const [selectedVehicle] = useState(0);
  const [activeCard, setActiveCard] = useState(0);
  const [expandPastAssignments, setExpandPastAssignments] = useState(false);
  const [selectedAssignment, setSelectedAssignment] = useState<string | null>(null);
  const [selectedPendingBinding] = useState(MOCK_PENDING_BINDING);
  ...
  // Initialise default scan binding from available bindings (run once on mount)
  useEffect(() => {
    const firstAvailable = BINDINGS.find(
      b => b.scanPayStatus === 'always_available' || b.scanPayStatus === 'in_window' || b.scanPayStatus === 'trip_window'
    ) ?? null;
    ...
    return (
      <SafeAreaView style={{ flex: 1, backgroundColor: '#fff' }}>
        ...
        >
                ...
                {Array.from({ length: 6 }).map((_, i) => (
                  <TextInput
                    ...
                    style={{
                      ...
                      borderColor: otpDigits[i] ? '#15803d' : '#d1d5db',
                    }}
                  />
                ))}
              </View>
          ...
          {/* Screen: Set PIN (new user or reset) */}
            ...
            <View>
              ...
              {inviteCode.length === 6 && INVITE_CODES_DB[inviteCode] && (
                <View style={{ backgroundColor: '#f0fdf4', borderWidth: 1, borderColor: '#86efac', borderRadius: 16, padding: 12, marginBottom: 16, flexDirection: 'row', gap: 12 }}>
                  <Check size={20} color="#15803d" style={{ flexShrink: 0 }} />
          ...
          {onboardingStep === 'forgot_otp' && (
            <View>
              ...
              <TouchableOpacity
                onPress={() => {
                  if (otp === '123456') {
                    setOtp(''); setNewPin(''); setPinConfirm(''); setPinError('');
                    setIsNewUser(false); setOnboardingStep('set_pin');
                  }
                }}
                disabled={otp.length !== 6}
                activeOpacity={0.7}
        ...
        <TouchableOpacity
          ...
          style={{ position: 'absolute', bottom: 80, right: 8, width: 32, height: 32, backgroundColor: '#e5e7eb', borderRadius: 16, alignItems: 'center', justifyContent: 'center', zIndex: 50 }}
          activeOpacity={0.7}
        >
          <Text style={{ color: '#6b7280', fontSize: 16 }}>⋮</Text>
        </TouchableOpacity>

        <Modal visible={showDevMenu} transparent animationType="fade" onRequestClose={() => setShowDevMenu(false)}>
          <Pressable style={{ flex: 1 }} onPress={() => setShowDevMenu(false)}>
            <View style={{ position: 'absolute', top: 16, right: 16, backgroundColor: '#1f2937', padding: 12, borderRadius: 8 }}>
              <TouchableOpacity onPress={() => { setOnboardingStep('complete'); setShowDevMenu(false); }} style={{ paddingVertical: 4 }}>
                <Text style={{ color: '#fff', fontSize: 12 }}>Skip to Main App</Text>
              </TouchableOpacity>
            </View>
          </Pressable>
        </Modal>
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#fff' }}>
      <View style={{ flex: 1 }}>
              ...
              <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'center', alignItems: 'center', padding: 16 }}>
                <View style={{ backgroundColor: '#fff', borderRadius: 16, padding: 24, gap: 16, width: '100%', maxWidth: 360 }}>
                  ...
                  <View style={{ flexDirection: 'row', gap: 12, marginTop: 8 }}>
                    ...
                    <TouchableOpacity
                      onPress={() => { setShowDeclineConfirm(false); setCurrentMainScreen('home_empty'); }}
                      activeOpacity={0.7}
              ...
              <View style={{ flexDirection: 'row', justifyContent: 'center', gap: 12 }}>
                {pairingDigits.map((digit, i) => (
                  <TextInput
                    ...
                    maxLength={1}
                    style={{
                      width: 44, height: 56, textAlign: 'center', fontSize: 20, fontFamily: 'monospace', fontWeight: '700',
                  ...
                  onPress={() => {
                    ...
                    if (code === validCode) {
                      ...
                      setTimeout(() => {
                        setPairingDigits(Array(6).fill(''));
                        setPairingError('');
                        pairingInputRefs.current[0]?.focus();
                      }, 1500);
                    }
                  }}
        ...
        {/* ── Main Screens (Home / Scan / Assignments / Transactions / Profile) ── */}
            ...
            <ScrollView style={{ flex: 1 }} contentContainerStyle={{ paddingBottom: 16 }} keyboardShouldPersistTaps="handled">
              ...
              {activeTab === 'card' && currentCard && (
                <View style={{ padding: 16, gap: 16 }}>
                  ...
                  {/* Scan & Pay status indicator */}
                  <View style={{
                    marginHorizontal: 16, paddingHorizontal: 16, paddingVertical: 10, borderRadius: 12, flexDirection: 'row', alignItems: 'center', gap: 8,
                    backgroundColor: (activeCards[activeCard]?.scanPayStatus === "always_available" || activeCards[activeCard]?.scanPayStatus === "in_window") ? '#f0fdf4' : '#fffbeb',
                    borderWidth: 1,
                    borderColor: (activeCards[activeCard]?.scanPayStatus === "always_available" || activeCards[activeCard]?.scanPayStatus === "in_window") ? '#bbf7d0' : '#fde68a',
                  }}>
                    <View style={{
                      width: 8, height: 8, borderRadius: 4,
                      backgroundColor: (activeCards[activeCard]?.scanPayStatus === "always_available" || activeCards[activeCard]?.scanPayStatus === "in_window") ? '#22c55e' : '#f59e0b',
                    }} />
                    <Text style={{
                      fontSize: 12, fontWeight: '600',
                      color: (activeCards[activeCard]?.scanPayStatus === "always_available" || activeCards[activeCard]?.scanPayStatus === "in_window") ? '#15803d' : '#b45309',
                    }}>
                      {activeCards[activeCard]?.scanPayStatus === "always_available" && "Scan & Pay always available"}
                      {activeCards[activeCard]?.scanPayStatus === "in_window" && activeCards[activeCard]?.authMode === "shift_based" && `Scan & Pay active · ends ${activeCards[activeCard]?.shiftEnd}`}
                      {activeCards[activeCard]?.scanPayStatus === "in_window" && activeCards[activeCard]?.authMode === "trip_linked" && `Scan & Pay active · ends ${activeCards[activeCard]?.tripEnd}`}
                    </Text>
                  </View>
                  ...
                  <View style={{ paddingHorizontal: 16 }}>
                    <TouchableOpacity
                      ...
                      activeOpacity={0.7}
                      style={{ backgroundColor: activeCards[activeCard]?.scanPayStatus === "out_window" ? '#e5e7eb' : '#16a34a', borderRadius: 16, paddingVertical: 14, alignItems: 'center', flexDirection: 'row', justifyContent: 'center', gap: 8 }}
                    >
                      <Svg width={20} height={20} viewBox="0 0 24 24" fill="none" stroke={activeCards[activeCard]?.scanPayStatus === "out_window" ? "#9ca3af" : "#fff"} strokeWidth={2} strokeLinecap="round">
                        <Rect x="3" y="3" width="7" height="7" />
                        <Rect x="14" y="3" width="7" height="7" />
                        <Rect x="3" y="14" width="7" height="7" />
                        <Path d="M14 14h3v3M17 20v1M20 14v3M20 20h1" />
                      </Svg>
                      <Text style={{ color: activeCards[activeCard]?.scanPayStatus === "out_window" ? '#9ca3af' : '#fff', fontWeight: '700', fontSize: 15 }}>Scan & Pay</Text>
                    </TouchableOpacity>
                  </View>

                  {/* Recent Transactions */}
                  <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 16, paddingTop: 8 }}>
                    <Text style={{ fontWeight: '700', fontSize: 16 }}>Recent</Text>
                    <TouchableOpacity onPress={() => setActiveTab('transactions')} activeOpacity={0.7}>
                      <Text style={{ fontSize: 14, color: '#16a34a', fontWeight: '600' }}>View all</Text>
                    </TouchableOpacity>
                  </View>
                  <View style={{ paddingHorizontal: 16, gap: 8 }}>
                    {mockTransactions.slice(0, 3).map((txn) => (
                      <View key={txn.id} style={{ backgroundColor: '#fff', borderWidth: 1, borderColor: '#e5e7eb', borderRadius: 12, padding: 12, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                        <View>
                          <Text style={{ fontSize: 14, fontWeight: '600', color: '#111827' }}>{txn.station}</Text>
                          {txn.type === 'Fueling' && <Text style={{ fontSize: 12, color: '#4b5563' }}>{currentCard.vrn}</Text>}
                          <Text style={{ fontSize: 12, color: '#4b5563' }}>{txn.date}</Text>
                        </View>
                        <Text style={{ fontSize: 14, fontWeight: '700', color: '#dc2626' }}>-₹{txn.amount}</Text>
                      </View>
                    ))}
                  </View>
                </View>
                ...
                <View style={{ padding: 16, gap: 16 }}>
                  {(() => {
                    const availableForScan = BINDINGS.filter(b =>
                      b.paired === true && b.state === "ACTIVE" &&
                      (b.scanPayStatus === "always_available" || b.scanPayStatus === "in_window" || b.scanPayStatus === "trip_window")
                    );
                    const lockedBindings = BINDINGS.filter(b =>
                      b.scanPayStatus === "locked_unpaired" || b.scanPayStatus === "locked_repair" || b.scanPayStatus === "out_window"
                    );
                      ...
                      return (
                        <View style={{ alignItems: 'center', justifyContent: 'center', paddingVertical: 48, gap: 24 }}>
                          ...
                          <View style={{ alignItems: 'center' }}>
                            <Text style={{ fontSize: 18, fontWeight: '700', color: '#111827', marginBottom: 4 }}>Scan & Pay unavailable</Text>
                            <Text style={{ fontSize: 14, color: '#4b5563' }}>No vehicles available for scanning right now</Text>
                          </View>
                          {lockedBindings.length > 0 && (
                            <View style={{ width: '100%', backgroundColor: '#f9fafb', borderRadius: 16, padding: 16, gap: 12 }}>
                              {lockedBindings.map(b => (
                                <View key={b.id}>
                                  <Text style={{ fontWeight: '600', color: '#111827', fontSize: 14 }}>{b.vrn}</Text>
                          ...
                          <TouchableOpacity onPress={() => setActiveTab('assignments')} activeOpacity={0.7} style={{ width: '100%', backgroundColor: '#15803d', borderRadius: 16, paddingVertical: 14, alignItems: 'center' }}>
                            <Text style={{ color: '#fff', fontWeight: '600' }}>Go to My Assignments</Text>
                          </TouchableOpacity>
                        </View>
                      );
                            ...
                            <View style={{ flexDirection: 'row', gap: 8 }}>
                              {availableForScan.map(b => (
                                <TouchableOpacity
                                  ...
                                  activeOpacity={0.7}
                                  style={{ paddingHorizontal: 12, paddingVertical: 6, borderRadius: 99, backgroundColor: selectedScanBinding?.id === b.id ? '#16a34a' : '#f3f4f6' }}
                                >
                                  <Text style={{ fontSize: 14, fontWeight: '600', color: selectedScanBinding?.id === b.id ? '#fff' : '#374151' }}>{b.vrn}</Text>
                                </TouchableOpacity>
                              ))}
                            </View>
                          </ScrollView>
                  ...
                  {sessionState === 'authorized' && (
                    <View style={{ gap: 16, marginTop: 8 }}>
                      ...
                      <View style={{ backgroundColor: '#f9fafb', borderRadius: 16, padding: 16, alignItems: 'center' }}>
                        ...
                        <Text style={{ fontSize: 24, fontWeight: '700', color: '#111827' }}>2.4 kg</Text>
                        <Text style={{ fontSize: 14, color: '#4b5563', marginTop: 4 }}>₹384</Text>
                      </View>
              ...
              {activeTab === 'assignments' && (
                <View style={{ padding: 16, gap: 16 }}>
                            ...
                            <View style={{ borderLeftWidth: 4, borderLeftColor: '#2563eb', backgroundColor: '#fff', borderRadius: 12, overflow: 'hidden', borderWidth: 1, borderColor: '#e5e7eb' }}>
                              ...
                              <View style={{ borderTopWidth: 1, borderTopColor: '#f3f4f6', flexDirection: 'row', gap: 12, paddingHorizontal: 20, paddingVertical: 12 }}>
                                ...
                                <TouchableOpacity onPress={() => { setShowTripDetails(true); setSelectedTripBinding(binding); }} activeOpacity={0.7} style={{ flex: 1, alignItems: 'center', gap: 4, paddingVertical: 8, backgroundColor: '#f9fafb', borderRadius: 12 }}>
                                  ...
                                  <Text style={{ fontSize: 12, fontWeight: '600', color: '#6b7280' }}>Trip info</Text>
                                </TouchableOpacity>
                              </View>
                            </View>
                          )}
                  ...
                  {(pendingBindings.length > 0 || repairBindings.length > 0) && (
                    <View style={{ gap: 12 }}>
                      <Text style={{ fontSize: 12, textTransform: 'uppercase', fontWeight: '700', color: '#d97706', letterSpacing: 1 }}>Needs attention</Text>
                      {pendingBindings.map((binding) => (
                        <View key={binding.id} style={{ borderWidth: 2, borderColor: '#fcd34d', borderStyle: 'dashed', backgroundColor: '#fff', borderRadius: 12, padding: 16, gap: 12 }}>
                          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                            ...
                            <View style={{ backgroundColor: '#fef3c7', paddingHorizontal: 12, paddingVertical: 4, borderRadius: 99 }}>
                              <Text style={{ fontSize: 12, fontWeight: '700', color: '#b45309' }}>Action needed</Text>
                            </View>
                          </View>
                          <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                            <Text style={{ fontSize: 14, color: '#4b5563' }}>Vehicle-linked</Text>
                            <Text style={{ fontSize: 14, fontWeight: '600', color: '#111827' }}>{binding.fo}</Text>
                          </View>
                          {binding.assignedBy && <Text style={{ fontSize: 12, color: '#4b5563' }}>Assigned by {binding.assignedBy} · 2h ago</Text>}
                          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                            <Lock size={16} color="#b45309" />
                            <Text style={{ fontSize: 14, color: '#b45309' }}>Pair to unlock Scan & Pay</Text>
                          </View>
                          <View style={{ borderTopWidth: 1, borderTopColor: '#f3f4f6', paddingTop: 12, gap: 8 }}>
                            <TouchableOpacity
                              ...
                              activeOpacity={0.7}
                              style={{ backgroundColor: '#15803d', borderRadius: 8, paddingVertical: 10, alignItems: 'center' }}
                            >
                              <Text style={{ color: '#fff', fontSize: 13, fontWeight: '600' }}>Accept & Pair</Text>
                            </TouchableOpacity>
                            <TouchableOpacity
                              onPress={() => setDeclineBndId(binding.id)}
                              activeOpacity={0.7}
                              style={{ borderWidth: 1, borderColor: '#fca5a5', borderRadius: 8, paddingVertical: 10, alignItems: 'center' }}
                            >
                              <Text style={{ color: '#dc2626', fontSize: 13, fontWeight: '600' }}>Decline</Text>
                            </TouchableOpacity>
                          </View>
                        </View>
                      ))}
                  ...
                  {(activeBindings.length > 0 || pendingBindings.length > 0 || repairBindings.length > 0) && (
                    <View style={{ gap: 12, paddingTop: 8 }}>
                      ...
                      >
                        ...
                        <ChevronDown size={16} color="#4b5563" style={{ transform: [{ rotate: expandPastAssignments ? '180deg' : '0deg' }] }} />
                      </TouchableOpacity>
        ...
        <TouchableOpacity
          onPress={() => setShowDevMenu(!showDevMenu)}
          style={{ position: 'absolute', bottom: 90, right: 8, width: 32, height: 32, backgroundColor: '#e5e7eb', borderRadius: 16, alignItems: 'center', justifyContent: 'center', zIndex: 50 }}
          activeOpacity={0.7}
        >
          <Text style={{ color: '#6b7280', fontSize: 16 }}>⋮</Text>
        </TouchableOpacity>
        <Modal visible={showDevMenu} transparent animationType="fade" onRequestClose={() => setShowDevMenu(false)}>
          <Pressable style={{ flex: 1 }} onPress={() => setShowDevMenu(false)}>
            <View style={{ position: 'absolute', top: 16, right: 16, backgroundColor: '#1f2937', padding: 12, borderRadius: 8 }}>
              <TouchableOpacity onPress={() => { setSessionState(sessionState === 'authorized' ? 'idle' : 'authorized'); setShowDevMenu(false); }} style={{ paddingVertical: 4 }}>
                <Text style={{ color: '#fff', fontSize: 12 }}>Toggle Auth State</Text>
              </TouchableOpacity>
            </View>
          </Pressable>
        </Modal>
